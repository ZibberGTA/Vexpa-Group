  ...
  String get adminHint {
    switch (this) {
      ...
      case _AdminPage.accountSupport:
        return 'Account help';
      case _AdminPage.venues:
        return 'Venue moderation';
      case _AdminPage.drinks:
        return 'Drink catalogue';
      case _AdminPage.deals:
...
class _AdminContent extends StatelessWidget {
  ...
  const _AdminContent({required this.role, required this.page, required this.pages, required this.onSelected});
  ...
  Widget _buildPage(BuildContext context) {
    ...
    switch (page) {
      case _AdminPage.dashboard:
        return _DashboardPanel(role: role, pages: pages, onSelected: onSelected);
      case _AdminPage.accountSupport:
...
class _DashboardPanel extends StatelessWidget {
  ...
  const _DashboardPanel({required this.role, required this.pages, required this.onSelected});

  @override
  Widget build(BuildContext context) {
    final hubPages = pages.where((page) => page != _AdminPage.dashboard).toList();

    return Column(
      key: const ValueKey('dashboard'),
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _PageHeader(title: 'Admin Control Centre', subtitle: 'Choose an area to manage. Access is filtered by staff role.'),
        LayoutBuilder(
          builder: (context, constraints) {
            final width = constraints.maxWidth;
            final columns = width > 1100 ? 4 : width > 760 ? 3 : width > 520 ? 2 : 1;
            return GridView.count(
              ...
              mainAxisSpacing: 12,
              crossAxisSpacing: 12,
              childAspectRatio: columns == 1 ? 4.5 : 3.1,
              children: hubPages
                  .map(
                    (page) => _AdminHubCard(
                      page: page,
          ...
          child: Row(
            children: [
              Container(
                ...
                height: 42,
                decoration: BoxDecoration(
                  color: const Color(0xFF9D28FF).withOpacity(0.12),
                  borderRadius: BorderRadius.circular(14),
                ),
                child: Icon(page.icon, color: const Color(0xFF9D28FF)),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(page.label, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
                    const SizedBox(height: 3),
                    Text(page.adminHint, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                  ],
                ),
              ),
...
class _TrailsPanel extends StatelessWidget {
  final StaffRole role;
  const _TrailsPanel({required this.role});

  @override
  Widget build(BuildContext context) {
    final canPublish = role.atLeast(StaffRole.management);

    return Column(
      key: const ValueKey('trails'),
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _PageHeader(title: 'Trails', subtitle: 'Generate, review and publish DrinkSpot Trails for users on the map.'),
        _InfoCard(
          title: 'Tonight\'s Trail workflow',
          lines: [
            'Generate a draft trail from venue activity, crowd levels, events and engagement.',
            'Review and adjust the venue stops before anything goes live.',
            if (canPublish) 'Publish or disable the active trail shown to users on the map.' else 'Admins can review trails. Publishing is available to Management and Founder roles.',
          ],
        ),
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Trail controls', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                const SizedBox(height: 12),
                Wrap(
                  spacing: 10,
                  runSpacing: 10,
                  children: [
                    FilledButton.icon(onPressed: () {}, icon: const Icon(Icons.auto_awesome), label: const Text('Generate draft')),
                    OutlinedButton.icon(onPressed: () {}, icon: const Icon(Icons.refresh), label: const Text('Regenerate')),
                    OutlinedButton.icon(onPressed: canPublish ? () {} : null, icon: const Icon(Icons.check_circle), label: const Text('Approve')),
                    FilledButton.icon(onPressed: canPublish ? () {} : null, icon: const Icon(Icons.publish), label: const Text('Publish')),
                    OutlinedButton.icon(onPressed: canPublish ? () {} : null, icon: const Icon(Icons.visibility_off), label: const Text('Disable')),
                  ],
                ),
              ],
            ),
