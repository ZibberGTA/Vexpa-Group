...
class VenueMapMarker {
  ...
  static Future<Uint8List> create({
    ...
    final key = '$venueId|$safeUrl|$glow|${glowColor?.value}|$size';
    ...
    if (glow) {
      final glowPaint = Paint()
        ..color = (glowColor ?? const Color(0xFFFF2D95)).withOpacity(0.42)
    ...
    if (safeUrl.isNotEmpty) {
      try {
        final data = await NetworkAssetBundle(Uri.parse(safeUrl)).load(safeUrl);
  ...
  static void _drawLetter(Canvas canvas, String venueName, Offset center, double w) {
    final letter = venueName.trim().isNotEmpty ? venueName.trim()[0].toUpperCase() : '?';
